...
The Postgresql database runs as a container. So make sure to install docker before running this example.
...
The `bamoe-mssql-mappings` library provides mapping files required by hibernate. You could take a look at the library 
[here](../../runtime-libraries/bamoe-database-support/bamoe-mssql-mappings/). This library is used in combination 
with `quarkus.hibernate-orm.mapping-files=META-INF/bamoe-data-index-orm.xml,META-INF/bamoe-job-service-orm.xml`
property which enables the example to use the mentioned orm XML files to configure the entity mapping.

The mssql database runs as a container. So make sure to install docker before running this example. In order to use 
mssql database as a Dev Service it also requires us to have a [license acceptance file](src/main/resources/container-license-acceptance.txt).
More on this [here](https://quarkus.io/version/3.15/guides/databases-dev-services#license_acceptance).
