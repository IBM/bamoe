...
This example shows the security features that come with BAMOE. It shows how to get an access token and how to use it to interact with the BAMOE business services. How to configure your application, and at least, it exemplifies how to use multiple IDPs in Management Console.
...
- `PROJECT_VERSION`: Should be set with the current version being used: `PROJECT_VERSION=<REPLACED_WITH_BAMOE_VERSION>`
...
First, start the Keycloak services (["Infrastructure requirements/Development mode"](#development-mode)), and then start the BAMOE business services in development mode. To do so, open three new terminals, access the service folders by using the `cd <project_path>` (kie-service-1/kie-service-2/kie-service-3) and run the command below on each terminal:
...
Start the Keycloak services (["Infrastructure requirements/JVM mode"](#jvm-mode)), and then open three new terminals to start the BAMOE business services with the following commands on each terminal:
...
### Requests to the BAMOE business service

With the access token in hands, you have access to the entire BAMOE business service API, you just need to pass the `Authorization` header. To get the processes:
...
### Connecting to the BAMOE business services in the Management Console
